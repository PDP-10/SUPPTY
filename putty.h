...
#define CONFIG_OPTIONS(X) \
    ...
    X(INT, NONE, serflow) \
	/* Supdup options */ \
	X(INT, NONE, supdup_mode) \
	X(STR, NONE, supdup_location) \
	X(INT, NONE, supdup_ascii_set) \
	X(INT, NONE, supdup_more) \
    X(INT, NONE, supdup_scroll) \
    /* Keyboard options */ \
...
#define TDINV	0777							// Not an official TD code, used to indicate an invalid/unset code.
