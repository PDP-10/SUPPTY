...
void term_copy_stuff_from_conf(Terminal *term)
{
	...
	term->supdup_mode = conf_get_int(term->conf, CONF_supdup_mode);
	term->supdup_ascii_set = conf_get_int(term->conf, CONF_supdup_ascii_set);
...
static unsigned long translate_supdup_extended_ascii(unsigned long in)
{
	static unsigned long waits_ascii[] = 
	{
		000000, 020623, 001661, 001662, 021047, 000254, 001665, 001700,
		001673, 000011, 000012, 000013, 000014, 000015, 021036, 021002,
		021202, 021203, 021051, 021052, 021000, 021003, 021227, 020624,
		000137, 020622, 000032, 021140, 021144, 021145, 021141, 021050,
	};

	static unsigned long its_ascii[] =
	{
		021031, 020623, 001661, 001662, 021047, 000254, 001665, 001700,
		001673, 001663, 001664, 020621, 000261, 022713, 021036, 021002,
		021202, 021203, 021051, 021052, 021000, 021003, 021227, 020624,
		020620, 020622, 021140, 022712, 021144, 021145, 021141, 021050,
	};

	if (in < 040)
	{
		return term->supdup_ascii_set ? waits_ascii[in] : its_ascii[in];
	}
	else
	{
		return 0;
...
static void term_out_supdup(Terminal *term)
{
	...
	while (nchars > 0 || unget != -1 || bufchain_size(&term->inbuf) > 0) 
	{
		...
		// TODO: The above is copied verbatim from term_out; should look into refactoring.
		...
		if (term->termstate == TOPLEVEL)
		{
			...
			if (c >= 0200)
			{
				...
				term->supdup.td_argindex = 0;
				term->supdup.td_code = c;
				switch (c)
				{
				case TDMOV:
					// %TD codes using 4 arguments
					term->supdup.td_argcount = 4;
					term->termstate = TD_ARGS;
					break;

				case TDMV0:
				case TDMV1:
					// %TD codes using 2 arguments
					term->supdup.td_argcount = 2;
					term->termstate = TD_ARGS;
					break;
				...
				case TDLF:
					...
					term->supdup.td_argcount = 0;
					term->termstate = TD_ARGSDONE;
					break;
			...
			{
				// Handle printable characters (all others)
				termline *cline = scrlineptr(term->curs.y);

				check_boundary(term, term->curs.x, term->curs.y);
				check_boundary(term, term->curs.x + 1, term->curs.y);

				/* Erase the current character at this position */				
				clear_cc(cline, term->curs.x);

				/*
				* Translate ITS/WAITS Extended ASCII characters into Unicode equivalents.
				*/				
				if (c == 026)
				{
					/*
					*Special case for "Circle-X" (extended character 026).
					* While there is a unicode glyph for this symbol, it is not present
					* in the commonly-available monospaced fonts on Windows systems.
					* To remedy this, we combine two characters(25CB "white circle"
					* and 00D7 "multiply sign")
					*/
					c = 0327;								// multiply sign
					add_cc(cline, term->curs.x, 022713);	// white circle
				}
				else if (c == 015)
				{
					/*
					* Special case for "Circle-Plus" (extended character 015).
					* As above it is not present in the commonly-available monospaced 
					* fonts on Windows systems.
					* To remedy this, we combine two characters(25CB "white circle"
					* and 002B "plus sign")
					*/
					c = 053;								// plus sign
				...
				{
					c = translate_supdup_extended_ascii(c);
				}				
		...
		else if (term->termstate == TD_ARGS)
		{
			// Collect up args for %TD code
			if (term->supdup.td_argindex < ARGS_MAX)
			{
				term->supdup.td_args[term->supdup.td_argindex] = c;
				term->supdup.td_argindex++;

				if (term->supdup.td_argcount == term->supdup.td_argindex)
				{
			...
			{
				// Should never hit this state, if we do we will just return to TOPLEVEL.
				term->termstate = TOPLEVEL;
			}
		}
		
		// If all arguments for a %TD code are ready, we will execute the code now.
		if (term->termstate == TD_ARGSDONE)
		{
			// Arguments for %TD code have been collected; dispatch based on the %TD code we're handling.
			switch (term->supdup.td_code)
			{
				case TDMOV:
					/*
					...
					*/
					...
					CLAMP(term->supdup.td_args[2], term->rows);
					CLAMP(term->supdup.td_args[3], term->cols);
					move(term, 
						term->supdup.td_args[3],
						term->supdup.td_args[2], 
						0);
					seen_disp_event(term);
					break;
				...
				case TDMV1:
					...
					positions.
					*/
					CLAMP(term->supdup.td_args[0], term->rows);
					CLAMP(term->supdup.td_args[1], term->cols);
					move(term,
						term->supdup.td_args[1],
						term->supdup.td_args[0],
						0);
					seen_disp_event(term);
					break;
				...
				case TDCRL:
					...
					*/
					...
					// Erase to EOL on the current line.
					erase_lots(
						term,
						TRUE, /* line only */
						FALSE, /* from cursor */
				...
				case TDQOT:
					...
					*/
					...
					// We just discard this, there's nothing we can do with this data,
					// unless we want to emulate an IMLAC.
					break;
				...
				case TDFS:
					...
					*/
					...
					// While I trust that ITS will not send a FS at the end of the line,
					// I'm going to check for it anyway.
					if (term->curs.y < term->rows)
					{
						move(term, term->curs.x, term->curs.y + 1, 0);
					}
					break;
				...
				case TDCLR:
					...
					*/
					erase_lots(
						term,
						FALSE, /* line only */
						TRUE, /* from beginning of screen */
						TRUE); /* to end of screen */
					seen_disp_event(term);
					move(term, 0, 0, 0);
					break;
					...
					*/					
					...
					if (term->curs.y <= term->marg_b)
						scroll(term, term->curs.y, term->marg_b,
							-def(term->supdup.td_args[0], 1), FALSE);
					seen_disp_event(term);
					break;
				...
				case TDICP:
					...
					*/
					CLAMP(term->supdup.td_args[0], term->cols);
					insch(term, def(term->supdup.td_args[0], 1));
					seen_disp_event(term);
					break;
				...
				/*
				* Non-standard (whatever "standard" means here) SUPDUP commands.
				* These are used (at the very least) by Genera's SUPDUP implementation.
				* Cannot find any official documentation, behavior is based on UNIX
				* SUPDUP implementation from MIT.
				*/
				case TDBS:
					/*
					* Backspace -- move cursor back one character (does not appear to wrap...)
					*/
					if (term->curs.x > 0)
					{
						term->curs.x--;
			...
			// Return to top level to pick up the next %TD code or printable character.
			term->termstate = TOPLEVEL;
		}		
	}
}
...
static void term_out(Terminal *term)
{
	...
	if (term->supdup_mode)
	{
		/* do SUPDUP logic elsewhere; no pointing in adding more noise to this 
		   already insanely long function. */
		term_out_supdup(term);
	}
	else
	{
		unget = -1;
