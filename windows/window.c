...
#define SHOW_TOASCII_RESULT
	...
	else /* supdup mode key handling */
	{
		int keyChar = 0;
		int top = 0;
		int handled = 0;

		// Handle function / special keys
		switch (wParam)
		{
			case VK_BACK:
				keyChar = 0177;		// RUBOUT
			...
			case VK_F5:
				keyChar = 0033;		// ALTMODE
				handled = 1;
				break;

			case VK_F6:
				keyChar = 0037;		// BACK NEXT
				handled = 1;
				break;			
		...
		// Not one of the special keys above, assume it is a printable character,
		...
		if (!handled)
		{
			...
			//
			// SUPDUP has no provisions for anything other than ASCII (or
			// MIT Extended ASCII -- either way, no UNICODE at all).
			// We will only take in keystrokes that return a single character
			// (dead keys for accents or diacritics are ignored).
			//
			if (r == 1)
			{
				keyChar = keys[0];
		...
		//
		// Mix in bucky bits if any are being pressed
		//
		if (handled)
		{
			...
			if (keystate[VK_F2] & 0x80)
			{
				// Symbolics SUPER
				keyChar |= 01000;
			}

			if (keystate[VK_F3] & 0x80)
			{
				// Symbolics HYPER
				keyChar |= 02000;
			}
		...
		//
		// If TOP is depressed then we need to generate the proper value in the range 0-37 (octal).
		//
		if (top)
		{
			//
			// Translate the ASCII code we already have in keyChar, if it is in the appropriate range:
			// 0100-0137 is the "uppercase" set
			// 0140-0177 is the "lowercase" set
			//
			if (keyChar >= 04100 && keyChar < 04177)
			{
				keyChar &= 04037;		// mask off TOP and the bottom 5 bits.
			}			
			else if (keyChar != 04177)
			{
				// We let 4177 past as is; otherwise we
				// can't apply TOP to this key.  Drop it.
				return 0;
			}
		}

		//
		// If bucky bits are present or we are sending 034 (escape) we need to handle it specially.
		//
		if (keyChar == 034)
		{
			// Escape the 034
		...
		else if (keyChar > 0177)
		{
			// Escape the modified character
			*p++ = 034;
			*p++ = ((keyChar >> 7) | 0100);
			*p++ = (keyChar & 0x7f);
		}
		...
		{
			// Not a recognized key or keystroke
			return 0;
		}
