				    ...
				    void *data, int event)
{
	...
	if (oldproto != newproto) {
		...
		/* If user selected SUPDUP, enable SUPDUP terminal mode, otherwise disable */
		conf_set_int(conf, CONF_supdup_mode, newproto == PROT_SUPDUP ? 1 : 0);
	}
		      ...
		      int protocol, int protcfginfo)
{
	...
	/*
	 * The Terminal/SUPDUP panel.  Only available if the session hasn't been started
	*/
	ctrl_settitle(b, "Terminal/SUPDUP",
		"Enabling and disabling SUPDUP user options");

	s = ctrl_getset(b, "Terminal/SUPDUP", "main", NULL);
	...
	if (!midsession)
	{
		...
		ctrl_radiobuttons(s, "Extended ASCII Character set:", 'z', 3,
			HELPCTX(supdup_ascii),
			conf_radiobutton_handler,
			I(CONF_supdup_ascii_set),
			"ITS", I(CHARSET_ITS),
			"WAITS", I(CHARSET_WAITS), NULL);
	...
	/* Window size cannot be changed midsession for SUPDUP */
	if (!( midsession && protocol == PROT_SUPDUP))
	{
	...
	if (!midsession) {
	    s = ctrl_getset(b, "Connection/SSH", "sharing", "Sharing an SSH connection between SuPuTTY tools");
	    ...
	    ctrl_checkbox(s, "Downstream (connecting to the upstream SuPuTTY)", 'd',
			  HELPCTX(ssh_share),
			  conf_checkbox_handler,
	...
	if (!midsession) {
	    ...
	    ctrl_droplist(s, "Chokes on SuPuTTY's SSH-2 'winadj' requests", 'j',
